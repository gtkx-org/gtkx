...
- **`v2ByteArrays`**: represents GIR byte sequences as `Uint8Array` instead of `number[]`. It covers `guint8` C arrays and `GByteArray`, in return values, out parameters, record fields, and properties, and it sets what the runtime's `fromVariant` unpacks a byte array variant (`ay`) to. It does not cover the handwritten Cairo overrides or the OpenGL bindings, which already use typed arrays.
