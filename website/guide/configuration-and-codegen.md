...
Every generated class, record, and interface that GObject registers a type for carries that type as a static `__gtype__`, so `Gtk.StringObject.__gtype__` says what `typeFromName("GtkStringObject")` used to, and `Gio.ListStore.new(Gtk.StringObject.__gtype__)` needs no string at all. It is read off the class it is accessed on rather than inherited, so a class [`registerClass`](/guide/subclassing) created reports its own type, and one that never went through `registerClass` reports `TYPE_INVALID`. A type GIR gives no `get-type` function carries no such static.

A record field becomes a getter where the read is known to land on the right memory, and a setter only where the write has somewhere to keep what it stores. A `null`-terminated array of pointers reads cleanly, so `Gio.DBusNodeInfo` hands back its `interfaces`, and it is read-only: a field slot cannot own an array handed to it, so there would be nothing keeping that array alive. Arrays stay out of the record's constructor props for the same reason. A field whose element count lives in a sibling field, and a `GList` or `GSList` link, get neither half, and are left off the class entirely rather than declared and left `undefined` at runtime.
