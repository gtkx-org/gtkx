...
GTKX recognizes `t`, `useTranslation`, `Trans`, `TransWithoutContext`, `IcuTrans`, and `IcuTransWithoutContext`, including imported or chained aliases, namespace and instance member calls, and CommonJS imports. Calls and explicit `i18nKey` props still need string-literal keys. Source defaults and contexts must also be statically known. A `Trans` component can derive its source from static children when `i18nKey` is omitted. Dynamic keys and tagged templates cannot be extracted.

Generated declarations under `node_modules/.gtkx` augment the `TranslationRegistry` exported by `@gtkx/i18n`. They check message keys and their required interpolation, count, and context options. Run codegen after adding messages; do not edit the declarations.
