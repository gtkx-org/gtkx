...
`dispose` and `finalize` run while GObject is taking the instance apart, where referencing it again is no longer allowed, so GTKX takes no reference to build the `this` they receive. This holds for every subclass, a widget as much as a plain `GObject`. When the object still has its wrapper, which is the case whenever something in JavaScript still holds the instance, that wrapper is what the override runs against and the fields it declares are all readable. An object whose wrapper was already collected reaches its own `dispose` and `finalize` on a fresh instance carrying none of them, so release state the collector cannot reach, such as a GLib source id, rather than assuming a field survived. That fresh instance lasts only for the call: it carries the handle `callParent` needs, and GTKX revokes it the moment the override returns, so a call on it afterwards, or on a handle read off it, throws instead of reaching freed memory.
