...
Props that are not GObject properties, such as the `accessible*` props, still animate: the component re-renders with the current value on each frame.
...
Elsewhere, animate the margins, the size requests, or a `Gtk.Paned`'s `position`.
