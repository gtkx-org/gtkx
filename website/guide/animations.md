...
Every widget of a `@gtkx/jsx` namespace the application imports from also hangs off `animated` directly: `animated.GtkLabel` is `animated(GtkLabel)` without an import of its own, and the two are the same component, so they can be mixed freely:
...
The properties cover the elements whose `ref` exposes a `Gtk.Widget` subclass. An element that is not a widget, such as `GtkAdjustment`, or a component of your own, is wrapped through the call form. Importing any element of a namespace brings in the whole of it, so one `@gtkx/jsx/gtk` import anywhere in the application is enough for every GTK widget to hang off `animated`; a namespace nothing imports is left out of the build, and its elements are not there to reach.
