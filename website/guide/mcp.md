---
description: "Give AI coding agents eyes and hands on your running app: the @gtkx/mcp server exposes the widget tree, queries, clicks, typing, and screenshots over the Model Context Protocol."
---

# MCP

An AI coding agent working on a web app can open the page and read the DOM. A native GTK window gives it nothing: no HTML to parse, no DevTools to attach. `@gtkx/mcp` closes that gap. It is an MCP (Model Context Protocol) server that connects any MCP client, Claude Code or otherwise, to your live GTKX app. Through it, an agent can list open windows, dump the widget tree, find widgets the way a test would, click buttons, type into entries, emit signals, and screenshot the result. Combined with the Fast Refresh loop of `gtkx dev`, this gives an agent the same edit, look, verify cycle you have as a human developer.
...
The server exposes eight tools, five for inspection and three for interaction:
...
::: info
Every tool call, inspection or interaction, is routed to the app with a 30 second timeout, so a hung app surfaces as a tool error rather than a stuck agent.
:::
...
6. You ask for a style tweak. The agent edits the component, `gtkx dev` applies it with Fast Refresh, and one more screenshot confirms the result, all without restarting the app or losing its state.
