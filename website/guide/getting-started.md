...
The watcher waits for a file to stop changing before it reports a save, so a patch lands about 50 ms after the last write instead of on the first one. That is what keeps a formatter's write followed by the editor's own write, or a `git checkout`, from patching a revision that is already superseded. The runner also re-reads the file once its module has loaded and loads it again when the contents moved in between, so what reaches the window is the revision on disk. A file that keeps changing through several of those attempts is left for the save that follows: `gtkx dev` says so on stderr rather than reporting a Fast Refresh it dropped.
