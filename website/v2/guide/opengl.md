...
`getShaderInfoLog` and `getProgramInfoLog` return the driver's diagnostics as a string. The [@gtkx/gl reference](/v2/reference/@gtkx/gl/) lists every command, enum, and type, including the `debugMessageCallback` and `clientWaitSyncLoop` overrides.
...
[Error Handling](/v2/guide/error-handling) covers matching the GErrors bindings throw.

## Next

Continue with [Testing](/v2/guide/testing) to see how the reconciler renders and asserts on widgets.
