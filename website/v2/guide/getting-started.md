...
description: "Scaffold an Adwaita-first GNOME app with npm create gtkx@beta, then run the GTKX development loop."
---

# Getting Started

The GTKX CLI scaffolds a native GNOME app on libadwaita and GTK4, installs its dependencies, and gives you a dev command that patches the running Adwaita window through Fast Refresh as you edit TypeScript.
...
minimumReleaseAgeExclude:
  - '@gtkx/react@2.0.0-beta.11'
  - '@gtkx/runtime@2.0.0-beta.11'
```
...
`npm run build` bundles the app to `dist/bundle.mjs`, and `npm start` runs that bundle with `node` on a machine carrying the Adwaita, GTK4, and GLib runtime libraries. `npm run deploy` goes further and packages the app as a Flatpak, a `.deb`, an `.rpm`, or an AppImage, with the desktop entry and AppStream metadata generated for you: see [Deploying](/v2/guide/deploying).
...
│  ├─ app.tsx            # Adwaita application window with a GTK counter
...
The starter's `<AdwApplication>`, imported from `@gtkx/jsx/adw`, picks up its `applicationId` from `gtkx.config.ts` automatically. It owns an `<AdwApplicationWindow>` and establishes Adwaita as the application layer; GTK widgets remain available inside it. [Your First Window](/v2/tutorial/your-first-window) builds that structure from scratch.
