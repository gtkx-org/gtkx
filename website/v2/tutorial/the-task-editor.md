...
`navigate` behaves here the way it did for the sidebar in [Lists and a Sidebar](/v2/tutorial/lists-and-the-sidebar). It pushes `Task` when no editor is open, and when one is it returns to the page already on the stack with the new params. Opening one task after another swaps what the single editor page shows instead of piling up editors you then have to back out of one at a time.
...
The lookup can miss, which is why the screen renders `null` rather than assuming a task. Deleting a task for good while its page is open is that case, and [Deleting Without Fear](/v2/tutorial/trash-and-toasts) pops the page as part of the delete.
...
`css` returns a generated class name, which is why it goes into `cssClasses` as a value rather than a string literal. The [CSS guide](/v2/guide/css) covers the details.
...
Deleting from here leaves the editor open over a task that is now in the trash. Leave that gap for now: [Deleting Without Fear](/v2/tutorial/trash-and-toasts) gives every delete an undo toast and a confirmation, and pops the page along the way.
...
[Menus, Accelerators, and Shortcuts](/v2/tutorial/actions-menus-shortcuts) turns the commands scattered across these buttons into GActions, puts them in a primary menu, and binds them to keys.
