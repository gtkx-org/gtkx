...
In [Adding Tasks with a Store](/v2/tutorial/the-task-store) you moved the task array into a Zustand store. This chapter adds a checkbox, a star, and a delete button to each row.
...
`accessibleLabel` names the icon-only controls for assistive technology and the queries used in [Testing](/v2/tutorial/testing).
