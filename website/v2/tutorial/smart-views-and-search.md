...
In [A Layout That Collapses](/v2/tutorial/an-adaptive-layout) the two panes learned to fold into one on a narrow window, and the content pane learned what to show when nothing is selected.
...
The view the app launches on needs rethinking too. A default selection is a default for the route, so it is `initialParams` rather than a field, and the value earns a name of its own in `src/navigation.ts`, since the commands in [Menus, Accelerators, and Shortcuts](/v2/tutorial/actions-menus-shortcuts) reach for it again:
...
The tooltip mentions a keyboard shortcut you build in [Menus, Accelerators, and Shortcuts](/v2/tutorial/actions-menus-shortcuts).
...
`filter` joined the UI slice on this page, so confirm the new field inherited the exclusion you established in [Lists and a Sidebar](/v2/tutorial/lists-and-the-sidebar). Leave it on **Done**, quit the app, and start it again: it comes back on **All**. The view comes back on All Tasks for a reason of its own, since navigation state is never written to disk either, so every launch starts at the `initialParams` you just changed.
...
[Opening a Task](/v2/tutorial/the-task-editor) gives a task a page of its own on the content stack, where it gains notes, a due date picked from a calendar, and an Important switch.
