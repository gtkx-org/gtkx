...
The sort order you added in [Preferences and the System Theme](/v2/tutorial/preferences-and-theming) includes Manual, which so far means the order tasks were created in. Make it mean the order you put them in.
...
The row has no way to work this out for itself. Since [Lists and a Sidebar](/v2/tutorial/lists-and-the-sidebar) the selection has been the `Tasks` route's params, which the screen owns and the list receives as a prop, and a row is neither. One computation per render of the list beats one lookup per row anyway.
...
[Reminders That Reach the Desktop](/v2/tutorial/reminders) sends a notification before a task is due, with buttons that still work once the app has closed.
