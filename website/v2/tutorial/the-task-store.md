...
In [Showing a List of Tasks](/v2/tutorial/a-list-of-tasks) you rendered a hardcoded array as an Adwaita boxed list, and the only way to change what was on screen was to edit the source. Now you make the list writable. A store holds the tasks, and a row at the top of the card creates one.
...
`addTask` trims the title and returns `null` when nothing is left, so pressing Enter on an empty entry creates nothing. It also returns the new id, which lets a caller open a task as soon as it exists in [Opening a Task](/v2/tutorial/the-task-editor).
