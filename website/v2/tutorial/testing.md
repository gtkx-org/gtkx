...
Extend the entry-row tests from [Add Tasks](/v2/tutorial/the-task-store#test-the-entry-row) to cover the completed app. Use `@gtkx/testing` to exercise it through the native accessibility tree, with real Adwaita widgets and no visible test window.
...
[Package the App](/v2/tutorial/packaging) adds the icon, desktop metadata, and installable packages.
