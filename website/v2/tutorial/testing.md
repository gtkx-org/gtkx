...
The [reminders chapter](/v2/tutorial/reminders) completed the app. Use `@gtkx/testing` to exercise it through the native accessibility tree, with real Adwaita widgets and no visible test window.
...
These cases cover an ordinary interaction, a blank input, and an unavailable drag operation. The drag payload is the task ID supplied by the app's `GtkDragSource`. The [complete example suite](https://github.com/gtkx-org/gtkx/blob/main/examples/tutorial/tests/tasks.test.tsx) also covers persistence, navigation, editing, deletion, and reordering.
...
The [testing guide](/v2/guide/testing) covers scoped queries, hooks, screenshots, and additional interactions. For Vitest configuration and assertions, use the [Vitest documentation](https://vitest.dev/guide/).

## Next

[Making It a Real Application](/v2/tutorial/packaging) adds the icon, desktop metadata, and installable packages.
