...
Keep the tasks from [Complete, Star, and Delete Tasks](/v2/tutorial/completing-and-deleting) between launches. Zustand handles persistence; a file adapter connects it to the GTKX application's data directory.

## A storage backend

Store task content in the application's data directory. Preferences use GSettings, introduced in [Add Preferences](/v2/tutorial/preferences-and-theming).
...
Wrap the store creator from [Add Tasks](/v2/tutorial/the-task-store) in `persist`. Nothing inside it changes.
...
Before running the tests from Add Tasks, give them a temporary data directory so they cannot overwrite tasks in your development app. Replace `vitest.config.ts` with:
...
Vitest runs this file before importing the app, so the storage backend reads the temporary path. The existing `beforeEach` in `tests/tasks.test.tsx` resets tasks between tests; those resets now also write to the temporary file. Keep `npm test` passing as you continue.
...
If you set `XDG_DATA_HOME`, use that directory instead. [Package the App](/v2/tutorial/packaging) covers running the same application in a Flatpak.

## Next

[Add Lists and a Sidebar](/v2/tutorial/lists-and-the-sidebar) splits the store into slices and gives tasks a list to belong to, reachable from a sidebar.
