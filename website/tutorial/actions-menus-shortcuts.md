...
[Edit Tasks](/tutorial/the-task-editor) added the editor route. Now give New Task one handler that its button, menu item, and accelerator can share. Add About and Keyboard Shortcuts dialogs alongside it.
...
`applicationId` comes from the generated project configuration, so the dialog and application shell use the same identity. The scaffold's `applicationIcon` configuration makes its icon available during development and in builds; [Package the App](/tutorial/packaging) installs it with the application.
...
[Add Undo and Delete Confirmation](/tutorial/trash-and-toasts) adds recoverable deletion and the New List dialog.
