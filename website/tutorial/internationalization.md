...
The [packaging chapter](/tutorial/packaging) made Tasks installable. Add a French catalog for its interface and desktop metadata.
...
Replace `src/format.ts` with the tutorial's [translated date helpers](https://github.com/gtkx-org/gtkx/blob/v1.6.0/examples/tutorial/src/format.ts). This keeps its existing exports and adds the plural message used by the test below. Gettext counts must be non-negative safe integers.
...
Keep the English tests from the [testing chapter](/tutorial/testing). Create `vitest.i18n.config.ts` for a separate French process:
...
[Shipping It on Flathub](/tutorial/flatpak) builds the localized Flatpak and prepares its source submission.
