...
In [Adding Tasks with a Store](/tutorial/the-task-store) you moved the task array into a zustand store, so typing a title into the top row appends a task. The rows themselves don't respond to clicks yet.
...
An icon-only button has no accessible name, so give icon-only controls an `accessibleLabel`. A screen reader announces it, and the tests in [Appendix A](/tutorial/testing) query by it, so a button without one is one no test can click.
