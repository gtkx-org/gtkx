...
Generation assumes one writer for a store directory. It builds the complete requested GI or GI-and-JSX set under a fresh generation, pins JSX to its sibling GI package, prepares the stable store and package links, and publishes the set with one atomic `current` symlink replacement. A failure before that replacement leaves the current generation unchanged. Up to three complete generations remain available for readers that still hold their paths. This machinery is in [`staging.ts`](https://github.com/gtkx-org/gtkx/blob/main/packages/codegen/src/staging.ts) and [`store/store-fs.ts`](https://github.com/gtkx-org/gtkx/blob/main/packages/codegen/src/store/store-fs.ts).

Changes to this machinery need an actual consumer requirement. [Production code must stay focused on consumers](/contributing/principles#keep-production-code-focused-on-consumers), so a quirk of GTKX's own workspace does not justify a production workaround. Do not add locks, process-identity checks, or multi-writer recovery while the supported execution model has one process handling a directory.
