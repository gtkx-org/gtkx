...
A four-file follow-up preserves intentional native menu props, snippet assembly without a generated child slot and synchronous tree-model factory returns. Ordinary menu action setup and the WebKit settings property observer now use JSX. Moving a snippet fixture's buffer ownership into JSX exposed text-mark cleanup criticals after the assertions completed. That tentative buffer change was withdrawn; its source lifecycle investigation remains open, and the failing path was not replayed.
...
A follow-up reads three complete CLI binding/type/throws suites, 848 lines, with 436 supporting helper and GIR lines. Older compiler wrappers still conflate abnormal termination with expected diagnostics, one declaration fixture bypasses the public package export, and implementation-string assertions remain in 17 binding cases and two callback-throws cases. Their replacement needs public compiler consumers and mapped native coverage; these test findings do not establish additional production defects.
...
The completed checkpoint passes full build, typechecking and lint; all 40 JSX consumer cases and 25 reference cases pass. Fresh installed TypeScript and JavaScript consumers pass. The tutorial passes 43 application cases, three French cases, localized packages and installed cold activation. All v2 guide and Contributing edits pass the website build. The latest native array checkpoint passes Rust checks, full types/lint, 1,687 ordinary native/runtime cases and 1,015 sanitizer cases; the normal addon is restored. The OSTree correction adds three passing ordinary and sanitizer consumer cases. The React repeat pass reaches 938 passing cases; the async correction passes 15 CLI consumers and 130 runtime/native controls. Shortcut constructors and seven isolated release fixtures pass. Every main CI check, CodeQL and the Sonar quality gate pass at `c820078f`; the subsequent local corrections await their own remote checkpoint.

Continue repeat audits alongside the remaining R2 container and ownership stages; string conversion and declarative notification activation are complete. Contiguous-byte output, callback seeds, list/bounded/terminated replacement, required shortcut constructors and GL descriptors are corrected and validated. Follow with explicit reference-counted container ownership, GL callback release, SourceView snippet/buffer cleanup, remaining CLI consumer contracts and desktop notification behavior. Keep the TextView, Sidebar, ComboRow, Cairo image-data and React Spring compatibility code until official upstream releases contain the fixes. Continue source and documentation audits after each coherent change; zero findings has not been reached and the remaining inventory still needs review.
