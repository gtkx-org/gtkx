...
Both conventions occur in introspectable libraries. OSTree's sign-engine result retains an object-unref destroyer, while AppStream's GI category helper explicitly clears its destroyer before returning. This is a source-confirmed descriptor mismatch for the OSTree result, not a runtime reproduction or an upstream attribution. Known received values may use full outer ownership with borrowed, independently retained leaves. Persistent replacement storage needs a separate explicit cleanup contract. Generic cleanup and a decision about the upstream metadata remain open; no shared-container mutation or external report was added.
...
The first twelve React integration files were read completely, 3,789 lines, from accessibility through custom elements. The remaining 31 files in that folder are still outside this repeat pass. Callback spies that observe supplied application handlers are distinct from replacements for external dependencies.
...
Accessibility cases now use public property, state and accessible-name matchers. Exact label assertions exposed two fixture mistakes: a generic role that prohibits naming, and two children competing for a single-child window. A group role and a shared box correct those fixtures without changing production matchers. All 34 cases pass, with affected types/lint and independent review. NotebookPage prop removal, useId rerender identity, navigation presence assertions and remaining imperative fixtures still need review.
...
The completed checkpoint passes full build, typechecking and lint; all 40 JSX consumer cases and 25 reference cases pass. Fresh installed TypeScript and JavaScript consumers pass. The tutorial passes 43 application cases, three French cases, localized packages and installed cold activation. All v2 guide and Contributing edits pass the website build. The latest native array checkpoint passes Rust checks, full types/lint, 1,687 ordinary native/runtime cases and 1,015 sanitizer cases; the normal addon is restored. Shortcut constructor consumer checks, all 102 updated control/application/accessibility cases and seven isolated release fixtures pass. The `fce4428b` remote checkpoint leaves the CLI job's old time limit and Sonar's ownership-transfer finding unresolved; their corrections await validation at the next pushed head.

Continue repeat audits alongside the remaining R2 container and ownership stages; string conversion and declarative notification activation are complete. Contiguous-byte output, callback seeds, list/bounded/terminated replacement, required shortcut constructors and GL descriptors are corrected and validated. Follow with explicit reference-counted container ownership, GL callback release, the remaining React test contracts and desktop notification behavior. Keep the TextView, Sidebar, ComboRow, Cairo image-data and React Spring compatibility code until official upstream releases contain the fixes. Continue source and documentation audits after each coherent change; zero findings has not been reached and the remaining inventory still needs review.
