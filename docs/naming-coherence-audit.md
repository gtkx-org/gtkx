...
- **`compounds-meta.ts`** holds codegen lookup tables (`CONTAINER_SLOTS`, `VIRTUAL_SUBCOMPONENTS`) and contains no `Metadata` identifier; the `*Metadata` family names typed GObject records. Different roles — renaming to `compounds-metadata.ts` would be cosmetic only.
