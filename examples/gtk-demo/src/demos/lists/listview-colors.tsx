...
interface FillSession {
    ...
    tickCount: number;
    lastTickAt: number;
    setColorsScheduled: number;
    setColorsFired: number;
}

const logFill = (fillId: number, fillStart: number, message: string): void => {
    console.error(`[gtkx:fill#${fillId}] +${Date.now() - fillStart}ms ${message}`);
};
...
const createFillTickCallback = (ctx: FillContext): (() => boolean) => {
    const { session, accumulated, targetLimit, increment, tickIdRef, setColors, setFilling } = ctx;
    return () => {
        const now = Date.now();
        const sinceLast = now - session.lastTickAt;
        ...
        if (session.tickCount <= 5 || done || session.tickCount % 256 === 0) {
            logFill(
                session.fillId,
                session.fillStart,
                `tick #${session.tickCount} dt=${sinceLast}ms size=${accumulated.length}/${targetLimit} done=${done}`,
            );
        }

        const scheduledSeq = ++session.setColorsScheduled;
        setTimeout(() => {
            session.setColorsFired++;
            if (scheduledSeq <= 3 || done || scheduledSeq % 256 === 0) {
                logFill(
                    session.fillId,
                    session.fillStart,
                    `setColors fire seq=${scheduledSeq}/${session.setColorsScheduled} fired=${session.setColorsFired}`,
                );
            }
            setColors(snapshot);
            if (done) {
                ...
                logFill(
                    session.fillId,
                    session.fillStart,
                    `FILL DONE ticks=${session.tickCount} scheduled=${session.setColorsScheduled} fired=${session.setColorsFired}`,
                );
            }
        }, 0);
...
const startGradualFill = ({
    ...
    const fillId = ++gradualFillCounter;
    const session: FillSession = {
        fillId,
        fillStart,
        tickCount: 0,
        lastTickAt: fillStart,
        setColorsScheduled: 0,
        setColorsFired: 0,
    };
    logFill(fillId, fillStart, `start targetLimit=${targetLimit} increment=${increment}`);
    tickIdRef.current = widget.addTickCallback(
        createFillTickCallback({ session, accumulated, targetLimit, increment, tickIdRef, setColors, setFilling }),
    );
};
