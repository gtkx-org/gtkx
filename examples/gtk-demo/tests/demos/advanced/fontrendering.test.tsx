...
describe("fontRenderingDemo metadata", () => {
    it("exposes the expected metadata", () => {
        expectDemoMetadata(fontRenderingDemo, {
            id: "fontrendering",
...
const findButtonByIcon = (root: Gtk.Widget, iconName: string): Gtk.Button | null => {
    const buttons = collectAll(root, (w): w is Gtk.Button => w instanceof Gtk.Button);
    return buttons.find((b) => b.getIconName() === iconName) ?? null;
};
...
describe("fontRenderingDemo zoom buttons", () => {
    it("renders zoom in and zoom out buttons", async () => {
        ...
        const zoomIn = findButtonByIcon(container, "list-add-symbolic");
        const zoomOut = findButtonByIcon(container, "list-remove-symbolic");
    ...
    it("invokes zoom out on click without disabling immediately", async () => {
        const { container } = await renderDemo(fontRenderingDemo);
        const zoomOut = findButtonByIcon(container, "list-remove-symbolic");
        if (!zoomOut) throw new Error("zoom out not found");
...
describe("fontRenderingDemo hint dropdown", () => {
    it("renders a dropdown for the hint-style selection", async () => {
        const { container } = await renderDemo(fontRenderingDemo);
        const dropdown = findFirst(container, (w): w is Gtk.DropDown => w instanceof Gtk.DropDown);
...
describe("fontRenderingDemo drawing area", () => {
    it("renders the drawing area inside a scrolled window", async () => {
        const { container } = await renderDemo(fontRenderingDemo);
        const drawingAreas = collectAll(container, (w): w is Gtk.DrawingArea => w instanceof Gtk.DrawingArea);
