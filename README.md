...
node-gtk does run on Node, but it is lightly maintained and predates modern Rust and N-API: its native addon is C++ on the older nan/V8 ABI, and it resolves every binding through libgirepository at runtime instead of from generated code. It ships untyped, has gaps in what it can marshal (its GHashTable support, for example, collapses keys to strings and cannot carry 64-bit, floating-point, or boolean values), and hand-merges the GLib and Node event loops in a way that diverges from how an ordinary GTK app is structured. Its documentation and examples still center on GTK3.
