...
This is the [`hello-world`](https://github.com/gtkx-org/gtkx/tree/main/examples/hello-world) example, verbatim. `@gtkx/gi` and `@gtkx/jsx` are per-project bindings generated by the CLI, not packages you install from npm.
...
GTKX runs on Node.js, not GJS. GJS is a separate JavaScript runtime, so building on it cuts you off from the enormous npm ecosystem of battle-tested packages and tooling. node-gtk does run on Node, but it is lightly maintained, predates modern Rust and N-API, has gaps in what it can marshal (its GHashTable support, for example, is minimal), manually merges the GLib and Node run loops in a way that diverges from how ordinary GTK apps are structured, ships untyped, and leans on libgirepository at runtime.

GTKX takes a different approach. A single generator reads GObject-Introspection and emits the TypeScript types and the native FFI calls together, in the same pass, for every method, function, and static. The types are never a hand-maintained afterthought, so they cannot drift from the calls they back, and they cover the full GTK4 and libadwaita surface rather than a hand-picked subset.
