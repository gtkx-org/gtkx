...
`pnpm codegen` reads GIR XML (e.g. `/usr/share/gir-1.0/Gtk-4.0.gir`) for the libraries in `gtkx.config.ts` and emits two generated packages into `node_modules/.gtkx/`: `@gtkx/gi` (FFI classes, property getters/setters, `connect`/`emit` switches) and `@gtkx/jsx` (one per-namespace module — `@gtkx/jsx/gtk`, `/adw`, … — carrying that namespace's `Props` types, the `React.JSX.IntrinsicElements` augmentation, and one `createElementComponent` line per element (HOC-wrapped for windows, dialogs, and applications) plus a `@gtkx/gi/<ns>` side-effect import, alongside one merged `@gtkx/jsx/metadata` holding the `SIGNALS`/`CONSTRUCT_ONLY_PROPS`/`DEFAULT_PROPS` tables plus the reconciler's data tables — codegen's built-ins merged with the project's `elementMap`/`arrayProps`/`slots`/`containerProps` config rows). `@gtkx/jsx` relates to `@gtkx/react` as `@gtkx/gi` relates to `@gtkx/ffi`: `@gtkx/react` is the namespace-agnostic runtime (`render`, `quit`, hooks, the reconciler, and the hand-written runtime components the namespace modules wrap with typed exports), and each `@gtkx/jsx/<ns>` builds on it. The import model is uniform: app code imports every element-like component from `@gtkx/jsx/<ns>` (including the enhanced ones — `GtkListView`, `GtkColumnView`, `GtkDrawingArea`, `AdwComboRow`, …), `render`/`quit`/hooks/shared types from `@gtkx/react`, animation components (`AnimatePresence`, `AdwTimedAnimation`, `AdwSpringAnimation`) from `@gtkx/animate`, widget enums/classes from `@gtkx/gi/<ns>`, and the resolved `gtkx.config.ts` fields (e.g. `applicationId`) from `@gtkx/config/runtime`, which re-exports the virtual module verbatim. The metadata reaches `@gtkx/react` through the `virtual:gtkx-config` module the gtkx Vite plugin serves, so importing only `@gtkx/jsx/gtk` never loads Adwaita. The pipeline entry is `packages/codegen/src/runner.ts` (`CodegenRunner`); per-namespace FFI emission is in `packages/codegen/src/ffi/pipeline.ts`, React emission in `packages/codegen/src/react/pipeline.ts`. Hand-written override templates (`packages/codegen/src/templates/<ns>/`) are raw `.ts` assets — excluded from codegen's own TS program — embedded into the store as `<ns>/overrides/*.js` plus the namespace barrel; they import `@gtkx/ffi` (which owns the listener tracking and shared runtime helpers) and generated modules, and the cairo overrides — which carry their own non-introspectable FFI helpers in `templates/cairo/ffi.ts` — also import `@gtkx/native` directly.
...
Module boundaries are enforced by `.dependency-cruiser.cjs`: no cycles; `@gtkx/native` is importable only by `ffi`, `react`, `codegen`; `@gtkx/codegen` references native via `import type` only, except its override templates which import it directly and run embedded in `@gtkx/gi`, and never imports `@gtkx/react` (the built-in reconciler tables flow the other way, through the generated metadata); `@gtkx/mcp` may depend only on `utils`; `@gtkx/utils` and `@gtkx/config` depend on no GTKX package.
...
- **Type names carry no module or namespace prefix.** Name a type for what it is (`ArgType`), not for the package it lives in (`FfiArgType`). Prefer building on the existing type a lower layer already defines over re-spelling its shape. When a local type specializes a base imported from another package that shares the name, import the base aliased as `Native<Name>` and give the local extension the bare name — e.g. `import { type ArgType as NativeArgType }` then `type ArgType = Readonly<NativeArgType> & { … }`.
...
- **GObject values in the React API:** props take raw GObject instances only, never React refs — a ref is still unpopulated during the commit's mutation phase. Hooks that observe a GObject (`useSignal`, `useProperty`, `useTickCallback`) take a `GObjectTarget<T>` (`packages/react/src/utils/gobject-target.ts`): the instance, a `RefObject` holding it, or `null`/`undefined` to stay inactive, resolved every render so the subscription follows the ref. Hooks return raw instances, never refs.
...
`src/index.tsx` renders the root component; the application component inside it owns the GTK application:
...
`src/app.tsx` is the root component; every element-like component (`GtkApplicationWindow`, `GtkBox`, `GtkColumnView`, `GtkMenuButton`, ...) is imported from `@gtkx/jsx/<ns>` (e.g. `@gtkx/jsx/gtk`, `@gtkx/jsx/adw`), `render`/`quit` and hooks from `@gtkx/react`, animation components (`AnimatePresence`, ...) from `@gtkx/animate`, widget enums and classes from `@gtkx/gi/<ns>`, and the resolved config fields (e.g. `applicationId`) from `@gtkx/config/runtime`. See `examples/hello-world` for the canonical reference, and `examples/gtk-demo`, `examples/tutorial`, `examples/browser` for richer apps.
