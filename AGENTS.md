...
`src/app.tsx` is the root component; every element-like component (`GtkApplicationWindow`, `GtkBox`, `GtkColumnView`, `GtkMenuButton`, ...) is imported from `@gtkx/jsx/<ns>` (e.g. `@gtkx/jsx/gtk`, `@gtkx/jsx/adw`), `render`/`quit` and hooks from `@gtkx/react`, animation components (`AnimatePresence`, ...) from `@gtkx/animate`, widget enums and classes from `@gtkx/gi/<ns>`, and the resolved config fields (e.g. `applicationId`) from the `virtual:gtkx-config` module. See `examples/hello-world` for the canonical reference, and `examples/gtk-demo`, `examples/tutorial`, `examples/browser` for richer apps.
