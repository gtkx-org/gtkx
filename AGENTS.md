...
1. **`@gtkx/react` — reconciler.** A `react-reconciler` host config (`packages/react/src/host-config.ts`) maps every tree operation onto plain `Instance` records (`packages/react/src/instance.ts`), each wrapping a backing GObject. Attach/detach dispatch is table-driven: serializable rows in the shapes `@gtkx/config` declares arrive pre-merged through `virtual:gtkx-config` (codegen's built-ins followed by the project's `elementMap`/`arrayProps` config rows) and are interpreted by `attach-rules.ts` and `array-props.ts`, while stateful relationships (slots, overlays, stack pages, menus, the widget-container fallback) stay hand-written interpreters in `element-map.ts`. Non-container relationships are slot/container-slot props on the parent element, not children — `addController`, `insertActionGroup`, and `layoutManager` on every widget, `addShortcut` on shortcut controllers, `addAction` on application windows — resolved per element by `createWidgetComponent` (`packages/react/src/create-widget-component.ts`), the factory behind every generated component, which walks the element's GType ancestry against the merged `SLOTS`/`CONTAINER_SLOTS` tables and splits those props into metadata wrapper children; nesting such an element as a child throws an error naming the prop. `applyProps` (`packages/react/src/nodes/internal/apply-props.ts`) diffs props into GObject property sets, signal connections, and prop-rule descriptors. `SignalStore` (`packages/react/src/nodes/internal/signal-store.ts`) owns all signal connections per root and, during a commit, suppresses blockable signal handlers except lifecycle signals (realize/map/destroy, etc.), which still fire.
...
`pnpm codegen` reads GIR XML (e.g. `/usr/share/gir-1.0/Gtk-4.0.gir`) for the libraries in `gtkx.config.ts` and emits two generated packages into `node_modules/.gtkx/`: `@gtkx/gi` (FFI classes, property getters/setters, `connect`/`emit` switches) and `@gtkx/jsx` (one per-namespace module — `@gtkx/jsx/gtk`, `/adw`, … — carrying that namespace's `Props` types, the `React.JSX.IntrinsicElements` augmentation, and one `createWidgetComponent` line per element (HOC-wrapped for windows, applications, dialog buttons, and actions) plus a `@gtkx/gi/<ns>` side-effect import, alongside one merged `@gtkx/jsx/metadata` holding the `SIGNALS`/`CONSTRUCT_ONLY_PROPS`/`DEFAULT_PROPS` tables plus the reconciler's data tables — codegen's built-ins merged with the project's `elementMap`/`arrayProps`/`slots`/`containerSlots` config rows). `@gtkx/jsx` relates to `@gtkx/react` as `@gtkx/gi` relates to `@gtkx/ffi`: `@gtkx/react` is the namespace-agnostic runtime (`render`, `quit`, hooks, the reconciler, and the hand-written enhanced components), and each `@gtkx/jsx/<ns>` builds on it. App code imports `render`/`quit` and enhanced components from `@gtkx/react`, animation components (`AnimatePresence`, `AdwTimedAnimation`, `AdwSpringAnimation`) from `@gtkx/animate`, intrinsics and namespace components from `@gtkx/jsx/<ns>`, and widget enums/classes from `@gtkx/gi/<ns>`. The metadata reaches `@gtkx/react` through the `virtual:gtkx-config` module the gtkx Vite plugin serves, so importing only `@gtkx/jsx/gtk` never loads Adwaita. The pipeline entry is `packages/codegen/src/runner.ts` (`CodegenRunner`); per-namespace FFI emission is in `packages/codegen/src/ffi/pipeline.ts`, React emission in `packages/codegen/src/react/pipeline.ts`. Hand-written override templates (`packages/codegen/src/templates/<ns>/`) are raw `.ts` assets — excluded from codegen's own TS program — embedded into the store as `<ns>/overrides/*.js` plus the namespace barrel; they import only `@gtkx/ffi` (which owns the non-introspectable helpers, e.g. `@gtkx/ffi/cairo` and the listener tracking) and generated modules, never `@gtkx/native`.
...
`src/app.tsx` is the root component; intrinsic elements (`GtkApplicationWindow`, `GtkBox`, ...) and namespace components are imported from `@gtkx/jsx/<ns>` (e.g. `@gtkx/jsx/gtk`, `@gtkx/jsx/adw`), enhanced components (`GtkColumnView`, `GtkMenuButton`, ...) from `@gtkx/react`, animation components (`AnimatePresence`, ...) from `@gtkx/animate`, and widget enums and classes from `@gtkx/gi/<ns>`. See `examples/hello-world` for the canonical reference, and `examples/gtk-demo`, `examples/tutorial`, `examples/browser` for richer apps.
