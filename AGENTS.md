...
| `@gtkx/config` | Single source of truth for `gtkx.config.ts`: schema, validation, loading and resolution, the containerProps/syntheticProps rules schema, the wrapper-protocol constants, and the Vite plugin that emits the `virtual:gtkx-config` module fusing resolved config with codegen metadata. |
