...
1. **`@gtkx/react` — reconciler.** A `react-reconciler` host config (`packages/react/src/host-config.ts`) maps every tree operation onto plain `Instance` records (`packages/react/src/instance.ts`), each wrapping a backing GObject. Attach/detach dispatch is table-driven: serializable rows in the shapes `@gtkx/config` declares arrive pre-merged through `virtual:gtkx-config` (codegen's built-ins followed by the project's `elementMap`/`arrayProps` config rows) and are interpreted by `attach-rules.ts` and `array-props.ts`, while stateful relationships (slots, overlays, stack pages, menus, the widget-container fallback) stay hand-written interpreters in `element-map.ts`. `applyProps` (`packages/react/src/nodes/internal/apply-props.ts`) diffs props into GObject property sets, signal connections, and prop-rule descriptors. `SignalStore` (`packages/react/src/nodes/internal/signal-store.ts`) owns all signal connections per root and, during a commit, suppresses blockable signal handlers except lifecycle signals (realize/map/destroy, etc.), which still fire.
