...
- **GObject values in the React API:** props take raw GObject instances only, never React refs — a ref is still unpopulated during the commit's mutation phase. Hooks that observe a GObject (`useSignal`, `useProperty`, `useTickCallback`) take a `GObjectTarget<T>` (`packages/react/src/gobject-target.ts`): the instance, a `RefObject` holding it, or `null`/`undefined` to stay inactive, resolved every render so the subscription follows the ref. Hooks return raw instances, never refs.
