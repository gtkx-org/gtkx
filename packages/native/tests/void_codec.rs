...
fn ptr_to_value_yields_undefined() {
    let from_null =
        unsafe { Decoder::read(&VoidCodec, ReadSource::Value(std::ptr::null_mut(), "ctx")) }
            .unwrap();
    assert!(matches!(from_null, Value::Undefined));

    let from_ptr =
        unsafe { Decoder::read(&VoidCodec, ReadSource::Value(8 as *mut c_void, "ctx")) }.unwrap();
...
fn read_from_pointer_yields_undefined() {
    ...
    let read = unsafe { Decoder::read(&VoidCodec, ReadSource::Slot(ptr, "ctx")) }.unwrap();
...
fn write_return_to_pointer_is_a_no_op() {
    ...
    PtrWriter::write_return_to_ptr(&VoidCodec, unsafe { Slot::new(ret) }, &Ok(Value::Undefined));
    PtrWriter::write_return_to_ptr(&VoidCodec, unsafe { Slot::new(ret) }, &Err(()));
    assert_eq!(slot, 99);
}
