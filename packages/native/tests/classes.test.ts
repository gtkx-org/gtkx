...
const registerCounterClass = (): bigint => {
    ...
    return registerClass(uniqueName(), objectType, {
        initialize: (klass) => {
            call(classInstallProperty, [klass, 1, intPspec("count")]);
        },
        vfuncs: [
            {
                ...
                fn: (_instance, _id, value) => {
                    stored = call(valueGetInt, [value]).value as number;
                },
            },
            {
...
test("a registered type reports the parent it was derived from", () => {
    ...
    expect(call(typeParent, [gtype]).value).toBe(objectType);
});
...
test("registering with no options and with empty options both derive from the parent", () => {
    ...
    expect(call(typeIsA, [withOptions, objectType]).value).toBe(1);
    expect(call(typeIsA, [withoutOptions, objectType]).value).toBe(1);
});
...
test("a subclass of a subclass derives from both of its ancestors", () => {
    ...
    expect(call(typeParent, [child]).value).toBe(parent);
    expect(call(typeIsA, [child, parent]).value).toBe(1);
    expect(call(typeIsA, [child, objectType]).value).toBe(1);
});
...
test("a registered class implements the interfaces it declares", () => {
    ...
    expect(call(typeIsA, [gtype, typePluginType]).value).toBe(1);
});
...
test("a construct property accepts complete GValue storage at an unaligned offset", () => {
    const owner = alloc(VALUE_SIZE + 1);
    const value = read(owner, { kind: "struct", ownership: "borrowed", isInline: true, size: VALUE_SIZE }, 1);

    copy(value as ExternalObject<Handle>, intValue(7), VALUE_SIZE);
...
test("a subclass of an abstract registered type is constructible", () => {
    const parent = registerClass(uniqueName(), objectType, { flags: 16 });
    const child = registerClass(uniqueName(), parent);
...
test("constructing an abstract registered type throws", () => {
    const gtype = registerClass(uniqueName(), objectType, { flags: 16 });
...
test("class initialization receives the registered GType before construction", () => {
    let initialized: bigint | undefined;
    const gtype = registerClass(uniqueName(), objectType, {
        initialize: (_handle, type) => {
            initialized = type;
        },
    });
    expect(initialized).toBe(gtype);
    expect(getType(construct(gtype).handle)).toBe(gtype);
});

test("a class initializer exception propagates to registration", () => {
    expect(() => registerClass(uniqueName(), objectType, { initialize: unreached })).toThrow();
});
