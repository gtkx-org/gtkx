...
fn null_guarded_runs_decode_for_non_null_pointer() {
    ...
    // SAFETY: the borrowed, size-less `StructDescriptor` wraps the non-null `ptr` as an unowned handle
...
fn write_object_ptr_writes_object_pointer() {
    ...
    // the pointer slot `write_value_to_pointer` stores the handle's object pointer into.
    unsafe {
