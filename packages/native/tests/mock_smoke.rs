...
fn value_getters_report_type_mismatch_statuses() {
    test_support::run(|| {
        ...
        let status = unsafe { sys::napi_get_value_double(env.raw(), string, &mut double_out) };
        assert_eq!(status, sys::Status::napi_number_expected);
        assert_eq!(double_out, 7.0);

        let mut int_out = 7;
        let status = unsafe { sys::napi_get_value_int32(env.raw(), string, &mut int_out) };
        assert_eq!(status, sys::Status::napi_number_expected);

        let mut uint_out = 7u32;
        let status = unsafe { sys::napi_get_value_uint32(env.raw(), string, &mut uint_out) };
        assert_eq!(status, sys::Status::napi_number_expected);

        let mut bool_out = true;
        let status = unsafe { sys::napi_get_value_bool(env.raw(), number, &mut bool_out) };
        assert_eq!(status, sys::Status::napi_boolean_expected);

        let mut length = 0usize;
        let status = unsafe {
            sys::napi_get_value_string_utf8(env.raw(), number, ptr::null_mut(), 0, &mut length)
        };
        ...
        let status = unsafe {
            sys::napi_get_value_bigint_int64(env.raw(), number, &mut int64_out, &mut lossless)
        };
        assert_eq!(status, sys::Status::napi_bigint_expected);

        let mut uint64_out = 0u64;
        let status = unsafe {
            sys::napi_get_value_bigint_uint64(env.raw(), number, &mut uint64_out, &mut lossless)
        };
        assert_eq!(status, sys::Status::napi_bigint_expected);

        let mut external_out: *mut c_void = ptr::null_mut();
        let status = unsafe { sys::napi_get_value_external(env.raw(), number, &mut external_out) };
...
fn property_access_on_null_or_undefined_reports_object_expected() {
    test_support::run(|| {
        ...
        let status = unsafe { sys::napi_get_named_property(env.raw(), undefined, key, &mut out) };
        ...
        let status = unsafe { sys::napi_get_element(env.raw(), undefined, 0, &mut out) };
        assert_eq!(status, sys::Status::napi_object_expected);

        let status = unsafe { sys::napi_set_element(env.raw(), null, 0, value) };
        assert_eq!(status, sys::Status::napi_object_expected);

        let status = unsafe { sys::napi_coerce_to_object(env.raw(), null, &mut out) };
        assert_eq!(status, sys::Status::napi_object_expected);
    });
}
...
fn array_length_on_a_non_array_reports_array_expected() {
    test_support::run(|| {
        ...
        let mut length = 0u32;
        let status = unsafe { sys::napi_get_array_length(env.raw(), object, &mut length) };
...
fn call_function_on_a_non_function_reports_function_expected() {
    test_support::run(|| {
        ...
        let mut out: sys::napi_value = ptr::null_mut();
        let status =
            unsafe { sys::napi_call_function(env.raw(), recv, target, 0, ptr::null(), &mut out) };
...
fn reference_unref_at_zero_reports_generic_failure() {
    test_support::run(|| {
        ...
        let status = unsafe { sys::napi_create_reference(env.raw(), object, 0, &mut ref_) };
        assert_eq!(status, sys::Status::napi_ok);

        let mut count = 42u32;
        let status = unsafe { sys::napi_reference_unref(env.raw(), ref_, &mut count) };
...
fn string_getter_with_zero_capacity_writes_nothing() {
    test_support::run(|| {
        ...
        let status = unsafe {
            sys::napi_get_value_string_utf8(env.raw(), value, buf.as_mut_ptr(), 0, &mut written)
        };
        assert_eq!(status, sys::Status::napi_ok);
        assert_eq!(written, 0);
...
fn read_bigint_handles_magnitudes_at_the_i128_boundary() {
    test_support::run(|| {
        ...
        let status = unsafe {
            sys::napi_create_bigint_words(env.raw(), 0, 2, words.as_ptr(), &mut positive)
        };
        assert_eq!(status, sys::Status::napi_ok);
        ...
        let status = unsafe {
            sys::napi_create_bigint_words(env.raw(), 1, 2, words.as_ptr(), &mut negative)
        };
        assert_eq!(status, sys::Status::napi_ok);
...
fn bigint_scalar_getters_wrap_and_report_losslessness() {
    test_support::run(|| {
        ...
        let status = unsafe {
            sys::napi_get_value_bigint_int64(env.raw(), two_pow_64, &mut int64_out, &mut lossless)
        };
        assert_eq!(status, sys::Status::napi_ok);
        ...
        unsafe {
            sys::napi_create_bigint_words(env.raw(), 1, 1, minus_one_words.as_ptr(), &mut minus_one)
        };
        let mut uint64_out = 0u64;
        lossless = true;
        let status = unsafe {
            sys::napi_get_value_bigint_uint64(env.raw(), minus_one, &mut uint64_out, &mut lossless)
        };
        assert_eq!(status, sys::Status::napi_ok);
        ...
        unsafe {
            sys::napi_create_bigint_words(env.raw(), 1, 1, int64_min_words.as_ptr(), &mut int64_min)
        };
        lossless = false;
        let status = unsafe {
            sys::napi_get_value_bigint_int64(env.raw(), int64_min, &mut int64_out, &mut lossless)
        };
        assert_eq!(status, sys::Status::napi_ok);
...
fn shared_views_expose_a_non_arraybuffer_backing_buffer() {
    test_support::run(|| {
        ...
        for (view, expected_plain) in [(shared, false), (plain, true)] {
            ...
            unsafe { sys::napi_is_arraybuffer(env.raw(), buffer, &mut is_arraybuffer) };
