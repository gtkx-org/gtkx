...
impl ModuleRequest for ConnectSignalRequest {
    ...
    fn execute(self) -> anyhow::Result<u64> {
        ...
        let closure = glib::RustClosure::new(move |values| data.marshal(values));
...
impl SignalClosureData {
    ...
    fn marshal(&self, values: &[glib::Value]) -> Option<glib::Value> {
        let outcome =
            std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| self.run_handler(values)));
        match outcome {
    ...
    fn default_return(&self) -> Option<glib::Value> {
        if self.return_gtype == glib::Type::UNIT || !self.return_gtype.is_valid() {
            return None;
        }
        Some(glib::Value::from_type(self.return_gtype))
    }

    fn run_handler(&self, values: &[glib::Value]) -> anyhow::Result<Option<glib::Value>> {
        if values.len() != self.arg_types.len() {
            bail!(
        ...
        for (value, ty) in values.iter().zip(&self.arg_types) {
            ...
            args.push(value.to_ir(ty, values)?);
        }

        let capture_result = !matches!(self.return_type, Type::Void(_));
        let (return_value, cells) = Mailbox::global()
            .invoke_node_and_wait_with_cells(&self.js_func, args, capture_result, out_cell_indices)
        ...
        Ok(Some(self.return_from_ir(&return_value)?))
    }
}
...
/// Read-side conversions from a marshalled signal [`glib::Value`] into the JS
/// IR, keyed on the value's fundamental `GType`.
trait GValueExt {
    /// The fundamental `GType` of the value, deciding which `g_value_get_*`
    /// accessor applies.
    fn fundamental(&self) -> glib::Type;

    /// Extracts the pointer payload of a pointer-carrying signal parameter.
    fn as_pointer(&self) -> anyhow::Result<*mut c_void>;

    /// Reads a 64-bit-capable scalar signal parameter as an exact IR bigint.
    /// Narrower promoted widths are accepted so a bigint-declared parameter
    /// marshalled as `G_TYPE_INT` / `G_TYPE_UINT` still converts.
    fn as_bigint(&self) -> anyhow::Result<i128>;

    /// Reads a scalar signal parameter as an IR number (signal marshalling
    /// promotes small integer widths to `G_TYPE_INT`/`G_TYPE_UINT`).
    fn as_scalar(&self) -> anyhow::Result<f64>;

    /// Converts this marshalled signal parameter to the JS IR, keyed on the
    /// codegen `descriptor`. `all_values` carries the full parameter list so a
    /// sized array can read its element count from the sibling argument its
    /// descriptor names.
    fn to_ir(&self, descriptor: &Type, all_values: &[glib::Value]) -> anyhow::Result<Value>;
}

impl GValueExt for glib::Value {
    fn fundamental(&self) -> glib::Type {
        fundamental_of(self.type_())
    }

    fn as_pointer(&self) -> anyhow::Result<*mut c_void> {
        let raw: *const gobject_ffi::GValue = self.to_glib_none().0;
    ...
    fn to_ir(&self, descriptor: &Type, all_values: &[glib::Value]) -> anyhow::Result<Value> {
        ...
        match descriptor {
            ...
            Type::Integer(_) | Type::Float(_) | Type::Tagged(_) => {
                self.as_scalar().map(Value::Number)
            }
            Type::BigInt(_) => self.as_bigint().map(Value::BigInt),
            Type::Unichar(_) => {
            ...
            Type::Array(array_type) => {
                ...
                if let crate::types::ArrayKind::Sized { size_index } = array_type.kind {
                    ...
                    let length = size_value.as_scalar()? as usize;
...
impl SignalClosureData {
    ...
    fn return_from_ir(&self, value: &Value) -> anyhow::Result<glib::Value> {
        let return_gtype = self.return_gtype;
        let descriptor = &self.return_type;
        ...
        unsafe {
            match fundamental {
                ...
                glib::Type::U_LONG => {
                    gobject_ffi::g_value_set_ulong(
                        raw,
                        narrowed_return(wide_int(value)?, "ulong")?,
                    );
                }
                glib::Type::I64 => {
                    gobject_ffi::g_value_set_int64(
                        raw,
                        narrowed_return(wide_int(value)?, "int64")?,
                    );
                }
                glib::Type::U64 => {
                    gobject_ffi::g_value_set_uint64(
                        raw,
                        narrowed_return(wide_int(value)?, "uint64")?,
                    );
                }
