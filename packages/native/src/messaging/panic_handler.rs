...
/// Runs `body`, catching any panic that would otherwise unwind across an
/// enclosing `extern "C"` boundary and abort the process. A caught panic is
/// reported to the [`ErrorReporter`] tagged with `context`, and `None` is
    ...
    match panic::catch_unwind(AssertUnwindSafe(body)) {
        ...
        Err(payload) => {
            ErrorReporter::global()
                .report_str(&format!("panic at {context}: {}", format_panic_payload(&*payload)));
            None
        }
    }
}
