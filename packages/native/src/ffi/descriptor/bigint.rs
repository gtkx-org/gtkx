...
impl BigIntKind {
    ...
    fn int_from_value(self, value: &value::Value) -> anyhow::Result<i128> {
        match value {
            ...
            _ => bail!(
                "Expected a BigInt for {} descriptor, got {value:?}",
                self.label()
            ),
        }
    }
    ...
    pub unsafe fn append_into(self, ptr: *mut u8, value: &value::Value) -> anyhow::Result<()> {
        ...
        // SAFETY: `stashed_value` is a range-checked 64-bit scalar and `ptr` addresses 8 writable
...
impl PointerWriter for BigIntKind {
    /// # Safety
    ///
    ...
    unsafe fn write_return_to_pointer(
        ...
        // SAFETY: `stashed_value` is a 64-bit scalar and `ret` addresses an 8-byte writable return
    ...
    unsafe fn write_value_to_pointer(
        ...
        // SAFETY: `stashed_value` is a range-checked 64-bit scalar and `ptr` addresses an 8-byte
...
mod tests {
    ...
    fn assert_field_round_trip(kind: BigIntKind, big: i128) {
        ...
        // preconditions of `write_value_to_pointer` and `read(ReadSource::Slot)` for a 64-bit kind.
        unsafe {
            kind.write_value_to_pointer(slot.as_mut_ptr().cast(), &value::Value::BigInt(big))
                .expect("write");
            let read = kind
    ...
    fn encode_accepts_bigint_beyond_2_53() {
        ...
        assert!(matches!(encoded, ffi::StashedValue::U64(v) if v == u64::MAX));
    }
    ...
    fn decode_i64_produces_exact_bigint() {
        let decoded = BigIntKind::I64
            .decode(&ffi::StashedValue::I64(i64::MIN))
            .expect("decode");
    ...
    #[test]
    fn to_stash_round_trips_through_as_bigint_vec() {
        for (kind, payload) in [
            (BigIntKind::I64, i128::from(i64::MIN)),
            (BigIntKind::U64, i128::from(u64::MAX)),
        ] {
            let storage = kind
                .to_stash(&[value::Value::BigInt(payload), value::Value::BigInt(7)])
                .expect("encode");
    ...
    #[test]
    fn to_stash_rejects_out_of_range_element() {
        assert!(
            BigIntKind::U64
                .to_stash(&[value::Value::BigInt(-1)])
                .is_err()
        );
        assert!(
            BigIntKind::I64
                .to_stash(&[value::Value::BigInt(i128::from(i64::MAX) + 1)])
                .is_err()
        );
    }
    ...
    #[test]
    fn decode_rejects_non_64bit_stashed_value() {
        assert!(BigIntKind::I64.decode(&ffi::StashedValue::I32(5)).is_err());
    }
    ...
    fn write_return_writes_value() {
        ...
        unsafe { BigIntKind::I64.write_return_to_pointer(slot.as_mut_ptr().cast(), &value) };
        // SAFETY: `slot` holds the 8-byte value just written, satisfying `read_i128`'s precondition.
    ...
    fn write_return_falls_back_to_zero_when_unrepresentable() {
        for (kind, payload) in [
            ...
            // SAFETY: `slot` is an 8-byte local buffer, satisfying the return-slot precondition.
            unsafe { kind.write_return_to_pointer(slot.as_mut_ptr().cast(), &value) };
