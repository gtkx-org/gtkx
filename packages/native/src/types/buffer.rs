...
impl FfiEncoder for BufferType {
    fn encode(&self, value: &value::Value) -> anyhow::Result<ffi::FfiValue> {
        match value {
            ...
            _ => bail!("Expected an ArrayBufferView, number, or null for buffer type, got {value:?}"),
        }
    }
...
impl FfiDecoder for BufferType {}

impl RawPtrWriter for BufferType {}
