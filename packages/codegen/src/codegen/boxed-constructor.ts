...
const isOpaque = (boxed: GirRecord): boolean => boxed.glibGetType === undefined && boxed.disguised;
...
export const renderBoxedConstructor = (context: ModuleContext, boxed: GirRecord, className: string): string => {
    if (isOpaque(boxed)) {
        return renderBlock(
            `constructor()`,
...
const allocArgs = (boxed: GirRecord, size: number): string[] => {
