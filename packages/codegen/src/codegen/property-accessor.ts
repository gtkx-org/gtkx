...
const renderGenericGetBody = (context: ModuleContext, property: GirProperty, tsType: string): string => {
    ...
    return `return getGobjectProperty(this, ${sourceStringLiteral(property.name)}, ${renderPropertyFfiType(context, property)}) as ${tsType};`;
};

const renderGenericSetBody = (context: ModuleContext, property: GirProperty): string => {
    context.addRuntimeImport("setGobjectProperty");
    context.addRuntimeImport("t");
    return `setGobjectProperty(this, ${sourceStringLiteral(property.name)}, ${renderPropertyFfiType(context, property)}, value);`;
};
