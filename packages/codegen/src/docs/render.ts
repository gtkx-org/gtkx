...
import {
    isFenceLine,
    stripManualMemoryManagement,
    stripStandaloneMemoryManagement,
} from "../writer/memory-management.js";
...
const callableDocMarkdown = (doc: string | undefined, note: string | undefined): string =>
    [docMarkdown(doc), note ?? ""].filter((text) => text.length > 0).join("\n\n");
...
const staticEntry = (options: StaticSectionOptions, callable: GirFunction): SignatureEntry | undefined => {
    ...
    return {
        ...
        doc: callableDocMarkdown(callable.doc, callableNote(options.context, callable, finishFn)),
...
const instanceMethodEntries = (
    ...
    for (const method of deduped) {
        ...
        entries.push({
            ...
            doc: callableDocMarkdown(method.doc, instanceMemberNote(signatureContext, method, scope)),
