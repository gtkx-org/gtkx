import type { ReactGiNamespaceFile } from "./react/pipeline.js";
import { type StoreOptions, writeStore } from "./store-fs.js";

/**
 * Options for assembling the injected `@gtkx/react-gi` package.
 */
export type ReactGiStoreOptions = StoreOptions & {
...
/**
 * Assembles the self-contained injected `@gtkx/react-gi` package: one module per
 ...
 * self-referential `@gtkx/react-gi` link lets a namespace module resolve a sibling
 * namespace's `Props` types through `@gtkx/react-gi/<ns>`; the `@gtkx/react` and
 ...
 */
export const writeReactGiStore = (
    options: ReactGiStoreOptions,
    namespaces: readonly ReactGiNamespaceFile[],
    metadata: string,
): void => {
    ...
    writeStore({
        ...
        symlinks: [
            ...
            { segments: ["node_modules", "@gtkx", "react-gi"], target: "self" },
