...
const commitInstrumentationOrigin = Date.now();
let commitCounter = 0;
let activeCommitId = 0;
let activeCommitStart = 0;
let activeCommitUpdateCount = 0;
let activeCommitMountCount = 0;
let activeCommitTextUpdateCount = 0;
const logCommitEvent = (id: number, event: string, details?: string): void => {
    const elapsed = Date.now() - commitInstrumentationOrigin;
    const suffix = details ? ` ${details}` : "";
    console.error(`[gtkx:commit#${id}] +${elapsed}ms ${event}${suffix}`);
};

const createCommitConfig = (): CommitConfig => ({
    commitUpdate: (instance, _type, oldProps, newProps) => {
        activeCommitUpdateCount++;
        return withSignalsBlocked(instance, () => instance.commitUpdate(oldProps, newProps));
    },
    commitMount: (instance) => {
        activeCommitMountCount++;
        instance.commitMount?.();
    },
    commitTextUpdate: (textInstance, oldText, newText) => {
        activeCommitTextUpdateCount++;
    ...
    prepareForCommit: () => {
        activeCommitId = ++commitCounter;
        activeCommitStart = Date.now();
        activeCommitUpdateCount = 0;
        activeCommitMountCount = 0;
        activeCommitTextUpdateCount = 0;
        logCommitEvent(activeCommitId, "prepareForCommit");
        freeze();
        return null;
    },
    resetAfterCommit: () => {
        const id = activeCommitId;
        logCommitEvent(
            id,
            "resetAfterCommit (drain start)",
            `updates=${activeCommitUpdateCount} mounts=${activeCommitMountCount} textUpdates=${activeCommitTextUpdateCount}`,
        );
        drainAfterCommit();
        logCommitEvent(id, "drainAfterCommit done");
        unfreeze();
        logCommitEvent(id, "unfreeze done", `dur=${Date.now() - activeCommitStart}ms`);
    },
});
