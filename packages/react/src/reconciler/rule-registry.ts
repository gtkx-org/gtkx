...
const capabilityCaches = new Map<RuleCapability, Map<GObject.GType, RuleSet | null>>();

const cacheFor = (capability: RuleCapability): Map<GObject.GType, RuleSet | null> => {
    const cached = capabilityCaches.get(capability);
    if (cached) return cached;
    const created = new Map<GObject.GType, RuleSet | null>();
    capabilityCaches.set(capability, created);
    return created;
};

const resolveRuleSetWith = (gtype: GObject.GType, capability: RuleCapability): RuleSet | null => {
...
export const resolveAppendRuleSet = (gtype: GObject.GType): RuleSet | null => resolveRuleSetWith(gtype, "appendChild");

export const resolveSetPropsRuleSet = (gtype: GObject.GType): RuleSet | null => resolveRuleSetWith(gtype, "setProps");
...
const ruleManagedCache = new Map<GObject.GType, Set<string>>();

const ruleManagedProps = (gtype: GObject.GType): Set<string> => {
