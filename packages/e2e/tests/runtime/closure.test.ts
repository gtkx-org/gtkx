...
describe("a value that cannot become a GObject.Closure", () => {
    ...
    it("is refused with the documented error even when it has no prototype", () => {
        expect(registerProbeWith(Object.create(null))).toThrow(ClosureMarshalError);
        expect(registerProbeWith(Object.create(null))).toThrow("Cannot marshal object into a GObject.Closure");
    });
});
