...
import { getWidgetText, render, screen, userEvent, waitFor } from "@gtkx/testing";
...
describe("userEvent.slide", () => {
    it("sets a Gtk.Scale to an exact value and clamps it to the adjustment's range", async () => {
        await render(
            <GtkScale
    ...
    it("throws when the target is not a range, and when a drag is used on one instead", async () => {
        await render(
            <GtkBox>
                <GtkButton label="nope" />
                <GtkScale adjustment={<GtkAdjustment value={0} lower={0} upper={100} />} />
            </GtkBox>,
        );

        const button = await screen.findByRole(Gtk.AccessibleRole.BUTTON, { name: "nope" });
        const scale = await screen.findByRole(Gtk.AccessibleRole.SLIDER, { as: Gtk.Scale });
        await expect(userEvent.slide(button, 5)).rejects.toThrow();
        await expect(userEvent.drag(scale, 50, 0)).rejects.toThrow();
    });
});
...
describe("userEvent.scroll", () => {
    it("drives a Gtk.ScrolledWindow's adjustments, including from a descendant", async () => {
        ...
        const scrolled = ref.current as Gtk.ScrolledWindow;
        const vadjustment = scrolled.getVadjustment();

        await waitFor(() => {
            expect(vadjustment.getUpper()).toBeGreaterThan(vadjustment.getPageSize());
        });

        await userEvent.scroll(scrolled, { y: 100 });
        ...
        await userEvent.scroll(scrolled, { x: 40 });
        expect(scrolled.getHadjustment().getValue()).toBe(40);
    });
    ...
    it("throws when there is no scrollable ancestor", async () => {
        ...
        await expect(userEvent.scroll(button, { y: 10 })).rejects.toThrow();
    });
});
