...
export class ImageSurface extends Surface {
    ...
    static create(format: Format, width: number, height: number): ImageSurface {
        return wrapHandle(cairoImageSurfaceCreate(format, width, height) as ExternalObject<Handle>, ImageSurface);
    }

    static createFromPng(filename: string): ImageSurface {
        return wrapHandle(cairoImageSurfaceCreateFromPng(filename) as ExternalObject<Handle>, ImageSurface);
    }
    ...
    getData(): Uint8Array {
        ...
        const ptr = getImageData(getHandle(this)) as ExternalObject<Handle> | null;
