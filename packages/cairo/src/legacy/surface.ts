...
SurfaceWithStatics.createSimilar = (other: Surface, content: Content, width: number, height: number): Surface => {
    return wrapHandle(cairoSurfaceCreateSimilar(getHandle(other), content, width, height) as ExternalObject<Handle>, Surface);
};
...
SurfaceWithStatics.createSimilarImage = (other: Surface, format: Format, width: number, height: number): Surface => {
    return wrapHandle(cairoSurfaceCreateSimilarImage(getHandle(other), format, width, height) as ExternalObject<Handle>, Surface);
};
...
SurfaceWithStatics.createForRectangle = (
    ...
    return wrapHandle(cairoSurfaceCreateForRectangle(getHandle(target), x, y, width, height) as ExternalObject<Handle>, Surface);
};
...
Surface.prototype.getDevice = function (): ExternalObject<Handle> | null {
...
Surface.prototype.mapToImage = function (extents: RectangleInt): Surface {
    return wrapHandle(cairoSurfaceMapToImage(getHandle(this), getHandle(extents)) as ExternalObject<Handle>, ImageSurface);
};
