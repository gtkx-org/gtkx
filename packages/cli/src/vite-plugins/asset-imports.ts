...
const QUERY_ADVICE =
    ...
    `query to keep ${RESOURCE_BINDING}`;

const RELATIVE_ADVICE =
    `Only assets imported through "${DATA_PREFIX}" are staged into the GResource bundle and export ` +
    `${RESOURCE_BINDING}; move the file under the data directory and import it as ${DATA_IMPORT_EXAMPLE}`;
...
const unbackedBinding = (parsed: ParsedModule): NamedBinding | undefined =>
    bindings(parsed).find(({ source }) => isAssetSpecifier(source) && !isDataAsset(source));

const bundleAdvice = (source: string): string => (source.startsWith(DATA_PREFIX) ? QUERY_ADVICE : RELATIVE_ADVICE);
