 ...
 * package and adds the project's resolved `gtkx.config.ts` as a `config`
 * constant. Delivering them through a virtual module keeps `@gtkx/react` free
 * of any dependency on `@gtkx/jsx`: the metadata flows in through the
 * bundler instead of a package import, so the dependency graph stays one-way
 * (`@gtkx/jsx` → `@gtkx/react`). `gtkx build` resolves and inlines the
