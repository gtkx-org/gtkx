...
describe("generated scalar C pointer omissions", () => {
    ...
    beforeAll(() => {
        project = cleanup.use(createScalarPointerProject(
            "gtkx-cli-scalar-pointer-inputs-",
            scalarPointerRejectedFiles(SCALAR_POINTER_REJECTED_INPUTS),
        ));
    });
    ...
    it.each(Object.keys(SCALAR_POINTER_REJECTED_INPUTS))("rejects the omitted public contract in %s", (name) => {
