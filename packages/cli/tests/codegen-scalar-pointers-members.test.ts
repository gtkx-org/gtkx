...
describe("generated scalar C pointer omissions", () => {
    ...
    beforeAll(() => {
        project = cleanup.use(createScalarPointerProject(
            "gtkx-cli-scalar-pointer-members-",
            scalarPointerRejectedFiles(SCALAR_POINTER_REJECTED_MEMBERS),
        ));
    });
    ...
    it.each(Object.keys(SCALAR_POINTER_REJECTED_MEMBERS))("rejects the omitted public contract in %s", (name) => {
