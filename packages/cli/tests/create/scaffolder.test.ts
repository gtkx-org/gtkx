...
describe("createScaffolder (src/* generated files)", () => {
    ...
    it("uses the supplied app id in src/application.ts", async () => {
        await runScaffolder();

        const content = vol.readFileSync(`${TEST_DIR}/test-app/src/application.ts`, "utf-8") as string;
        expect(content).toContain('new Gtk.Application({ application_id: "org.test.app" })');
    });
