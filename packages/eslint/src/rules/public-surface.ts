...
const manifestsFor = (root: string): Map<string, { dir: string; manifest: Manifest }> => {
    const found: Map<string, { dir: string; manifest: Manifest }> = new Map();
...
const readSourceCondition = (value: unknown): string | undefined => {
    if (typeof value === "string") {
        return value;
    }

    const conditions = value as { source?: unknown } | null;
    const source = typeof conditions === "object" && conditions !== null ? conditions.source : undefined;

    return typeof source === "string" ? source : undefined;
};

const resolveEntryFiles = (root: string, entrypoints: string[]): string[] => {
    const manifests = manifestsFor(root);

    return entrypoints.flatMap((specifier) => {
        const { name, subpath } = splitSpecifier(specifier);
        const entry = manifests.get(name);
        const source = readSourceCondition(entry?.manifest.exports?.[subpath]);

        return entry === undefined || source === undefined ? [] : [resolve(entry.dir, source)];
    });
};
